...
N_GENES <- N_GRNAS
N_CELLS_PER_BATCH <- 10000

# generate gene and grna ids
gene_ids <- paste0("gene-", seq(1, N_GENES))
grna_ids <- c(paste0("GENE-TARGET-", seq(1, N_PC_GRNAS)),
              paste0("NTC-", seq(1, N_GRNAS - N_PC_GRNAS)))

syn_data <- lapply(X = seq(1, 2), FUN = function(batch_id) {
  # assign the gRNAs to cells; the gRNAs are drawn according to different probabilities
  grna_probs <- rbeta(n = N_GRNAS, shape1 = 1, shape2 = 1.5)
  grna_probs <- grna_probs/sum(grna_probs)
  grna_assignments <- sample(x = seq(1, N_GRNAS), size = N_CELLS_PER_BATCH, replace = TRUE, prob = grna_probs)
  
  # set the mean expression level of the genes; the PC genes have a different mean expression, while the remaining genes have the same mean expression
  mu_0s <- mu_1s <- numeric(N_GENES)
  mu_0s[seq(1, N_PC_GRNAS)] <- rgamma(n = N_PC_GRNAS, shape = 2, rate = 0.5)
  mu_1s[seq(1, N_PC_GRNAS)] <- rgamma(n = N_PC_GRNAS, shape = 2, rate = 0.5)
  mu_0s[seq(N_PC_GRNAS + 1, N_GENES)] <- mu_1s[seq(N_PC_GRNAS + 1, N_GENES)] <- rgamma(n = N_GENES - N_PC_GRNAS, shape = 2, rate = 0.5)
  thetas <- runif(n = N_GENES, min = 1, max = 25)
  ...
  # append the row names
  rownames(gene_expression_mat) <- gene_ids
  rownames(grna_expression_mat) <- grna_ids
  
  # return the data
  list(gene_expression_mat = gene_expression_mat, grna_expression_mat = grna_expression_mat, grna_assignments = grna_assignments)  
})

# combine the matrices into a single matrix
gene_expression_mat <- cbind(syn_data[[1]]$gene_expression_mat, syn_data[[2]]$gene_expression_mat)
grna_expression_mat <- cbind(syn_data[[1]]$grna_expression_mat, syn_data[[2]]$grna_expression_mat)
grna_assignments <- c(syn_data[[1]]$grna_assignments, syn_data[[2]]$grna_assignments)

# create the batch vector
batch <- c(rep("b1", N_CELLS_PER_BATCH), rep("b2", N_CELLS_PER_BATCH)) |> factor()

# generate the gene ids, grna ids, and cell barcodes; append to the ondisc objects
cell_barcodes <- paste0("cell-", seq(1, N_BATCHES * N_CELLS_PER_BATCH))
colnames(gene_expression_mat) <- cell_barcodes
colnames(grna_expression_mat) <- cell_barcodes
...
# update the grna odm (with target info) and gene odm and grna odm (with batch info)
grna_odm <- grna_odm |>
  mutate_feature_covariates(grna_tbl) |>
  mutate_cell_covariates(batch = batch)
save_odm(odm = grna_odm, metadata_fp = paste0(sceptre2_dir, "data/simulated/experiment_2/grna_expression/metadata_orig.rds"))
gene_odm <- gene_odm |>
  mutate_cell_covariates(batch = batch)
save_odm(odm = gene_odm, metadata_fp = paste0(sceptre2_dir, "data/simulated/experiment_2/gene/metadata_orig.rds"))
