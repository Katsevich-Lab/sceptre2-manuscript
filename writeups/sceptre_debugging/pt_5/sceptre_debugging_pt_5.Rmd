...
undercover_res_sde <- readRDS(paste0(result_dir, "undercover_grna_analysis/undercover_result_grp_size_1.rds")) |>
  dplyr::select(-clock_time, -max_ram) |>
...
What is going on with those pairs that exhibit miscalibration? Do they have small effective sample sizes?

```{r}
problem_pairs <- undercover_res_w_ess |>
  filter(p_value < 1e-4, pass_qc) |>
  select(response_id, undercover_grna) |>
  summarize(paste0(undercover_grna, response_id)) |>
  pull()
```
