...
# 2. undercover res
undercover_res <- readRDS(paste0(result_dir, "undercover_grna_analysis/undercover_result_grp_size_1_sceptre.rds")) |>
...
# Part 3: Why is Seurat DE poorly calibrated on the Schraivogel data?


```{r}
# compute the row-wise KS stats
x <- mw_debug |> select(-z_null) |> select(starts_with("z_"))
ks_stats <- apply(X = x, MARGIN = 1, FUN = function(row) {
  vect <- as.numeric(row)
  fit <- ks.test(x = vect, pnorm)
  fit$statistic
})
ks_fits <- cut(x = ks_stats,
              breaks = unique(unlist(ks_stat_categories)),
              labels = names(ks_stat_categories))
mw_debug <- mw_debug |> mutate(ks_stat = ks_stats,
                               ks_fit = ks_fits)
```
...
Plot some of the Papalexi MW fits

```{r}
mw_debug_papa <- mw_debug |> filter(dataset == "papalexi_eccite_screen_gene")
