...
This is the third writeup in the SCEPTRE debugging series. The broad purpose of this series is to answer two overarching questions: (1) what are the analysis challenges, and (2) how can we address them? I ask four specific questions in this writeup.

1. To what extent does applying a more stringent QC threshold *actually* make the problem easier? (Answer: applying more stringent QC makes the problem easier on the Frangieh datasets but not on the others.)

2. Do the Seurat resampling distributions resemble the SCEPTRE resampling distributions in easy and challenging problem settings?

3. Might latent confounding explain the miscalibration we observe in the Schraivogel perturb-seq data? (Answer: Not really.)

4. How do the *exact* (as opposed to *skew-normal*) SCEPTRE p-values look? Are they well-calibrated? (Answer: the exact SCEPTRE p-values and )
...
We do see a difference here. On the Frangieh datasets (which are fairly sparse), the QC'ed pairs demonstrate much better calibration than the randomly downsampled pairs. Finally, we plot the results for SCEPTRE.
...
Including the top PC(s) does seem to improve the calibration of negative binomial regression slightly; by contrast, the calibration of SCEPTRE does not appear to change. Overall, the direction of searching for latent confounding via unsupervised learning does not appear to be especially promising.
...
**Frangieh**: The Frangieh data are fairly straightforward. There is no confounding. SCEPTRE and Seurat DE exhibit inflation when the effective sample size is small, as the asymptotic approximations that these methods make to the null distribution of the test statistic are not very good. As we apply more stringent QC (and thus as the effective sample size grows), SCEPTRE and Seurat DE exhibit better calibration. The Fisher exact test --- a non-asymptotic test of independence --- is well-calibrated in all settings.

**Papalexi**: Not totally clear, but we are close. Importantly, QC does **not** seem to make the problem easier, suggesting that something other than sparsity is causing challenges. Also, as far as calibration quality goes, SCEPTRE $\approx$ Fisher exact > Seurat DE. SCEPTRE is slightly worse than . An important question to answer is thus as follows: why is Seurat DE (in contrast to SCEPTRE and Fisher exact) so poorly calibrated on the Papalexi data?

**Schraivogel**: The most mysterious data. Basically none of the methods (except for perhaps MIMOSCA) are calibrated on the TAP-seq or perturb-seq data. I have explored various possible causes of miscalibration, including latent confounding and NTC heterogeneity. None were conclusive. Another idea is to check the gRNA assignments; if some cells received both a non-targeting gRNA and a targetin gRNA, this could cause signal to creep into the undercover pipeline.
