...
Why do we observe this difference between the Frangieh datasets on one hand and the Papalexi and Schraivogel datasets on the other? The Frangieh datasets are sparser than the Papalexi and Schraivogel datasets, and so applying QC results in a much larger change in the composition of the pairs analyzed. Below, I plot the percent decrease in the number of pairs per dataset after applying QC. The datasets that *do* use targeted sequencing (i.e., Schraivogel enhancer screen and Schraivogel TAP-seq) exhibit a small percentage decrease, whereas those that *do not* use targeted sequencing (the Frangieh and Papalexi datasets) exhibit a large percentage decrease.
...
# Is there unmeasured confounding in the Schraivogel TAP-seq and perturb-seq datasets?
