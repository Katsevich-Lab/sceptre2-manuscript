...
I seek to answer four outstanding questions in this writeup:
...
3.  Why are all methods poorly calibrated on the Schraivogel perturb-seq data?
...
fisher_exact_res_plot |>
              filter(dataset %in% c("frangieh_co_culture_gene", "frangieh_control_gene",
                                    "schraivogel_ground_truth_tapseq_gene", "schraivogel_enhancer_screen",
                                    "simulated_experiment_1_gene", "papalexi_eccite_screen_gene",
                                    "schraivogel_ground_truth_perturbseq_gene", "frangieh_ifn_gamma_gene",
...
The Fisher exact test yields a *p*-value exactly equal to one for a large fraction of the negative control pairs. What is the reasons for this? I investigate the properties of pairs with *p*-values exactly equal to one.
...
# link up with the ntc effective sample size df
fisher_w_samp_size <- fisher_exact_res_plot |> filter(dataset %in%
                                                c("frangieh_co_culture_gene",
...
x <- fisher_w_samp_size |>
  mutate(p_is_1 = (p_value == 1)) |>
  group_by(p_is_1, dataset) |>
  summarize(mean_ess = mean(effective_samp_size),
            median_ess = median(effective_samp_size)) |>
  mutate(p_is_1 = ifelse(p_is_1, "yes", "no"))

mean_ess_compare <- pivot_wider(data = x |> select(-median_ess),
                                names_from = "p_is_1",
                                values_from = "mean_ess")
median_ess_compare <- pivot_wider(data = x |> select(-mean_ess),
                                names_from = "p_is_1",
...
grna_odm_fp <- paste0(dataset_fp, "/grna_assignment/matrix.odm")
grna_metadata_fp <- paste0(dataset_fp, "/grna_assignment/metadata_qc.rds")
grna_odm <- read_odm(grna_odm_fp, grna_metadata_fp)
cellwise_targets <- lowmoi::get_target_assignments_via_max_op(grna_odm)
gene_odm <- gene_odm[,cellwise_targets == "non-targeting"]
grna_odm <- grna_odm[(grna_odm |> get_feature_covariates() |> dplyr::pull(target)) == "non-targeting",
                     cellwise_targets == "non-targeting"]
Z <- gene_odm |> get_cell_covariates() |> dplyr::select(dplyr::starts_with("pc"))
n_ntcs <- nrow(grna_odm)

ps <- sapply(seq(1, n_ntcs), function(i) {
  y <- grna_odm[[i,]] |> as.numeric()
  data_mat <- Z |> dplyr::mutate(y = y)
  fit_alt <- glm(formula = y ~ ., family = "binomial", data = data_mat)
  fit_null <- glm(formula = y ~ 1, family = "binomial", data = data_mat)
  s <- lmtest::lrtest(fit_alt, fit_null)
  s$`Pr(>Chisq)`[2]
})
sceptre::make_qq_plot(ps)
```
...
```{r}
fisher_enh_screen <- fisher_exact_res_plot |>
  filter(dataset == "schraivogel_enhancer_screen")
m <- nrow(fisher_enh_screen)
fisher_pert_seq <- fisher_exact_res_plot |>
  filter(dataset == "schraivogel_ground_truth_perturbseq_gene") |>
  sample_n(size = m, replace = FALSE)
to_plot <- rbind(fisher_enh_screen, fisher_pert_seq)

ggplot(data = to_plot,
             mapping = aes(y = p_value, col = dataset)) +
   stat_qq_points(ymin = 1e-10, size = 0.8) +
   geom_abline(col = "darkred") +
   stat_qq_band() +
   theme_bw() +
...
The perturb-seq dataset is not all that different from the enhancer screen dataset.
...
to_plot |>
  ...
  ggplot(mapping = aes(x = effective_sample_size,
                       fill = p_lt_001)) +
  geom_density(alpha = 0.8) + facet_wrap(dataset_rename_w_pairs ~ .,
                            scales = "free",
                            labeller = label_wrap_gen(35)) +
  xlab("Effective sample size") +
  ylab("p-value") +
  ...
  labs(color = "p < 0.001") +
  geom_vline(aes(xintercept = qc_thresh))
```
...
-   We have roughly 9.5 weeks until December 15. Assuming that we still are aiming for that deadline, can we come up with a week-by-week plan to get a manuscript preprinted by then? Leaving 3 weeks to making figures and writing, this leaves about 6.5 weeks for finalizing results.
