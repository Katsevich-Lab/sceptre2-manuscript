...
# 5. Fisher exact
fisher_exact_res <- readRDS(paste0(result_dir,
...
First, I plot the SCEPTRE exact results on the Frangieh data, comparing to (i) the skew-normal version of SCEPTRE (with covariates), (ii) NB regression, and (iii) SCEPTRE (SN). I plot the results twice: once without QC (top) and once with mild QC (bottom). ("Mild QC" entails filtering for pairs with at least 7 treatment cells with nonzero expression and 30 control cells with nonzero expression. The exact thresholds that I selected are somewhat arbitrary and can be fine-tuned later, if necessary.)
...
comparison_df <- left_join(x |>
  ...
  mutate(pass_qc = (n_nonzero_treatment >= 7) & (n_nonzero_control >= 100))
