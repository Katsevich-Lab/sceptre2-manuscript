...
Next, I study the relationship between standard normal fit quality and effective sample size. I plot standard normal fit quality (as quantified by KS statistic) against effective sample size for a set of pairs sampled from the Papalexi dataset.
...
I plot the empirical p-value against the MW p-value.

```{r}
mw_debug_select |>
  mutate(inflation = p_value < p_emp) |>
  ggplot(aes(x = p_value,y = p_emp, col = inflation)) +
  geom_point() +
  theme_bw() +
  scale_x_log10() +
  scale_y_log10() +
  ...
  geom_hline(yintercept = 2e-5, col = "darkblue")
```
