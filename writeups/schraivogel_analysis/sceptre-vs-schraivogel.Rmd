---
title: "Comparing SCEPTRE and Schraivogel method on Schraivogel data (updated)"
author: "Gene and Tim"
date: "2023-04-08"
output: pdf_document
---
...
results_merged <- result_sceptre |> 
  ...
  left_join(result_schraivogel |> 
              rename(p_value_schraivogel = p_value,
                     logFC_schraivogel = logFC), 
            by = c("response_id", "grna_group")) |>
  na.omit() |>
  rowwise() |>
  mutate(`Positive control` =  grepl(pattern = "-TSS$", x = grna_group)) |>
  ungroup() |>
  arrange(desc(`Positive control`))
```
...
\textcolor{blue}{SCEPTRE no longer returns p-values equal to zero; the smallest p-values are on the order of 1e-75 and correspond to positive controls.}
...
\textcolor{blue}{There is no longer a massive spike near zero; the p-values are uniform, as would be expected in an enhancer-targeting screen.}
...
\textcolor{blue}{No longer the case.}
...
\textcolor{blue}{It is still not totally clear to me why there is a cross shape; I'd have to look into how Schraivogel Method computes log fold changes.}