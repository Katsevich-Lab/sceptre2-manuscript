...
This analysis aims to see if Seurat and SCEPTRE differ on finding general TF targets. I will create a table with the odds ratio and corresponding pvalues for each method across TFs. The results are below and show that while both methods perofrm similarly, SCEPTRE outperforms Seurat slightly on STAT1,IRF1,and SMAD4. 
...
odds_ratios <- matrix(NA, nrow = length(TF_names), ncol = 4, 
                      dimnames = list(ground_truth = c("STAT1","STAT2","BRD4",
                                                       "MYC","SMAD4","IRF1"), 
  method = c("SCEPTRE", "Seurat","Pvalue-SCEPTRE","Pvalue-Seurat")))
```
...
for(TF_name in TF_names){
  ...
  database_genes <- read_table(paste0(data_dir,
  ...
  odds_ratios[TF_name, "SCEPTRE"] <- merged_results |> 
    select(in_database, sceptre_discovery) |>
    table() |>
    fisher.test() %>%
    `$`("estimate")
  
  odds_ratios[TF_name, "Seurat"] <- merged_results |> 
    select(in_database, seurat_discovery) |>
    table() |>
    fisher.test() %>%
    `$`("estimate")
  
  odds_ratios[TF_name, "Pvalue-SCEPTRE"] <- merged_results |> 
    select(in_database, sceptre_discovery) |>
    table() |>
    fisher.test() %>%
    `$`("p.value")
  
  odds_ratios[TF_name, "Pvalue-Seurat"] <- merged_results |> 
    select(in_database, seurat_discovery) |>
    table() |>
    fisher.test() %>%
    `$`("p.value")
