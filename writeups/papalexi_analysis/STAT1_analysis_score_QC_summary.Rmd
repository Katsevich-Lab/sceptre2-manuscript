---
title: "STAT1 SCEPTRE vs Seurat With Score QC Summary"
output: pdf_document
date: "2023-04-04"
header-includes:
- \usepackage{diagbox}
...
In this writeup I will change the method of QC for the CHIP-seq data to be such that peaks are only called if they are in the top $\alpha$ percentile of peaks with respect to the score reported. I will return the odds ratios that SCEPTRE and Seurat give with respect to the post-QC chipseq data. For more information such as the number of discoveries, see "STAT1-analysis-score-QC-individual".
...
for(alpha in alphas){
  score_quant = quantile(chipseq_data_copy$score,as.numeric(alpha))
  chipseq_data = chipseq_data_copy%>%filter(score > score_quant)
...
odds_ratios |>
  ...
  Hmisc::latex(booktabs = TRUE,
           ...
           file="",
           caption = "Enrichment odds ratios, comparing to ChIP-seq target assignments with changing pvalue quantile threshold",
           where = "!htbp")
```
