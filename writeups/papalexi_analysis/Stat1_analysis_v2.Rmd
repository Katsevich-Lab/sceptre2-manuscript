---
title: "STAT1 SCEPTRE vs Seurat in monocytes, with same QC"
output: pdf_document
date: "2023-04-05"
header-includes:
- \usepackage{diagbox}
...
chr = varhandle::unfactor(TSS@seqnames@values)
chip = list()
...
sceptre_results_STAT1 <- sceptre_results |>
...
# apply BH correction to Seurat STAT1 results
seurat_results_STAT1 <- seurat_results |>
  filter(grna_group == "STAT1", response_id %in% genes_qc) |>
...
merged_results <- chipseq_scores |>
  ...
  left_join(sceptre_results_STAT1, by = "gene") |>
  left_join(seurat_results_STAT1, by = "gene")
```
...
This table suggests that SCEPTRE and Seurat results have decent, but imperfect, agreement. Also note that Seurat makes roughly 800 more discoveries than SCEPTRE. Next, let's look at how the SCEPTRE and Seurat discoveries align with the ChIP-seq target genes (as identified by either the hTFtarget database or directly from the ChIP-seq data).
...
The proportions are the proportion of `TRUE` values in each row or column. For example, 0.742 of the genes found by SCEPTRE are marked as STAT1 targets in the database (i.e. SCEPTRE has specificity 0.742). From these tables, we see that **SCEPTRE has slightly lower sensitivity and slightly higher specificity than Seurat.** We can summarize each 2-by-2 table via its p-value and odds ratio.

```{r, results = 'asis'}
odds_ratios <- matrix(NA, nrow = 2, ncol = 2, 
                      dimnames = list(ground_truth = c("database", "ChIP-seq"), 
                                      method = c("SCEPTRE", "Seurat")))
enrichment_pvals <- matrix(NA, nrow = 2, ncol = 2, 
                      dimnames = list(ground_truth = c("database", "ChIP-seq"), 
                                      method = c("SCEPTRE", "Seurat")))
...
enrichment_pvals["database", "SCEPTRE"] <- merged_results |> 
  select(in_database, sceptre_discovery) |>
  table() |>
  fisher.test() %>%
  `$`("p.value")
...
enrichment_pvals["database", "Seurat"] <- merged_results |> 
  select(in_database, seurat_discovery) |>
  table() |>
  fisher.test() %>%
  `$`("p.value")
...
enrichment_pvals["ChIP-seq", "SCEPTRE"] <- merged_results |> 
  select(binary_score, sceptre_discovery) |>
  table() |>
  fisher.test() %>%
  `$`("p.value")
...
enrichment_pvals["ChIP-seq", "Seurat"] <- merged_results |> 
  select(binary_score, seurat_discovery) |>
  table() |>
  fisher.test() %>%
  `$`("p.value")
...
enrichment_pvals |> 
  ...
  Hmisc::latex(booktabs = TRUE,
           rowlabel="\\diagbox{Ground truth}{Method}", 
           file="",
           caption = "Enrichment p-values, comparing to database and our ChIP-seq target assignments.",
           where = "!htbp")
```
...
These results suggest that **there is moderate enrichment of ChIP-seq signal in both the SCEPTRE and Seurat discoveries, presumably because CD14+ cells are a better match to THP1 cells. The enrichment odds ratios for SCEPTRE are modestly higher than those of Seurat.**
