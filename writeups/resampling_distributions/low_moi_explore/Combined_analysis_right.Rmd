...
library(ggplot2)
library(tidyverse)
undershoot <- read_csv("undershoot_joint_fit.csv")[,-1]
overshoot <- read_csv("overshoot_joint_fit.csv")[,-1]
...
for (r in 1:10){
  dist <- abs(mle_overshoot_ratio[331:660] - quantile(mle_overshoot_ratio[331:660], quantile_list[r]))
  overshoot_set[r, 1] <- which(dist == min(dist))
  overshoot_set[r, 2] <- mom_overshoot_ratio[which(dist == min(dist)) + 330]
  dist <- abs(mle_undershoot_ratio[331:660] - quantile(mle_undershoot_ratio[331:660], quantile_list[r]))
...
```{r, warning = FALSE, error = FALSE, message = FALSE}
undershoot <- read_csv("undershoot_joint_power.csv")[,-1]
overshoot <- read_csv("overshoot_joint_power.csv")[,-1]
quantile_list <- seq(0.01, 0.99, length.out = 10)
no_sam <- round(exp(seq(log(1e3), log(5e4), length.out = 10)))
# rearrange the data frame
B <- 100
