  ...
  for (j in 1:1) {
    ...
    for (l in 1:nrow(resampling_dists)){
      ...
      for(tail in c("left", "right")){
        ...
        sim_norm <- rnorm(1e6, mean = mu, sd = sigma)
        sim_norm_pos <- sim_norm*sign_flip
        trun_norm <- sim_norm_pos[sim_norm_pos > u_pos]
        
        # employ ks.test
        test_ks <- ks.test(trun_data, trun_norm)
        print(is.nan(test_ks$p.value))
        print(test_ks$p.value)
        ...
        if(tail == "left"){
          ...
          gpd_tail_prob <- as.vector(pnorm(q = observed_extreme_quantile,
                                           mean = mu,
                                           sd = sigma))
        }else{
          observed_tail_prob <- (10:(0.1*n))/n
  ...
  for (j in 1:1) {
    ...
    for (l in 1:nrow(resampling_dists)){
      ...
      for(tail in c("left", "right")){
        ...
        trun_sknorm <- sim_sknorm_pos[sim_sknorm_pos > u_pos]
