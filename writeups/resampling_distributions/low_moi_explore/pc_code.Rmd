...
for (i in 1:length(q_list)) {
  for (j in 1:1) {
    ...
    subDir <- sprintf("figures/truncnorm_tail_%d_resamples_%.2f_percentile",  samples, q)
    dir.create(file.path(working_dir, subDir))
    ...
    for (l in 1:nrow(resampling_dists)){
      ...
      for(tail in c("left", "right")){
        ...
        fit_trunc <- fit.norm(rnorm(trun_data))

        mu <- fit_trunc$mu
        sigma <- fit_trunc$sig
        # first fit ks.test with estimated parameter; if there is Na, switch to simulation
        test_ks <- ks.test(trun_data, "ptruncnorm", a = u_pos, mean = mu, sd = sigma)
        print(is.nan(test_ks$p.value))
        print(test_ks$p.value)
        if(is.nan(test_ks$p.value)){
        ...
        if(tail == "right"){
          dgpd_density <- function(x) {
          (1 - q) * dtruncnorm(x, a=u_pos, b=Inf, mean = mu, sd = sigma)
        }
        }else{
          dgpd_density <- function(x) {
          (1 - q) * dtruncnorm(x, a=-Inf, b=-u_pos, mean = mu, sd = sigma)
        }
        }
        ...
        expected_quantiles_pos <- qtruncnorm(p = (2*(1:n)-1)/(2*n),
                                             a = u_pos,
                                             mean = mu,
                                             sd = sigma)
        expected_unif <- ptruncnorm(q = sort(sign_flip*z_null_pos[z_null_pos > u_pos]),
                                    a = u_pos,
                                    mean = mu,
                                    sd = sigma)
        ...
        observed_tail_prob <- (10:(0.1*n))/n
        observed_extreme_quantile <- quantile(trun_data, 1 - observed_tail_prob)
        gpd_tail_prob <- 1 - as.vector(ptruncnorm(q = observed_extreme_quantile,
                                                  a = u_pos,
                                                  mean = mu,
                                                  sd = sigma))
        # plot the tail probability
