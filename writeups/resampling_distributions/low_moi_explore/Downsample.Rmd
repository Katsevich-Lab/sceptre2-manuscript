...
undershoot <- list()
for (s in 1:10) {
  overshoot_sim <- data.frame(replicate_id = rep(1:B, length(no_sam)),
                              overshoot_ratio = numeric(length(no_sam)*B),
                              subsample = rep(no_sam, each = B))
  undershoot_sim <- data.frame(replicate_id = rep(1:B, 10), 
  ...
  for(t in 1:10){
    for (r in 1:B) {
      ...
      # undershoot case
      undershoot_sim[(t-1)*B + r, 2] <- max(tail_list / (1 - psn(under_quantile_emp, dp = under_dp_fit)))
      
      # overshoot case
      overshoot_sim[(t-1)*B + r, 2] <- max((1 - psn(over_quantile_emp, dp = over_dp_fit)) / tail_list)
      print(undershoot_sim[(t-1)*B + r, 2])
      print(overshoot_sim[(t-1)*B + r, 2])
    }
  }
  overshoot[[s]] <- overshoot_sim
  undershoot[[s]] <- undershoot_sim
  print(s)
}

# save file
write.csv(overshoot, "overshoot.csv")
write.csv(undershoot, "undershoot.csv")
