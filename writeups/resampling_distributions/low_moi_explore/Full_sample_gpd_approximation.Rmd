  ...
  for (j in 1:1) {
    ...
    for (l in 1:nrow(resampling_dists)){
      ...
      for(tail in c("left", "right")){
        ...
        # simulate sample from distribution with parameter in out
        sim_sknorm <- sn::rsn(1e6, dp = out@param$dp)
        sim_sknorm_pos <- sim_sknorm*sign_flip
        
        trun_sknorm <- sim_sknorm_pos[sim_sknorm_pos > quantile(sim_sknorm_pos, q)]
        
        # ks.test
        test_ks <- ks.test(trun_data, trun_sknorm)
        print(is.nan(test_ks$p.value))
        print(test_ks$p.value)
        ...
        hist_full <- tibble(z_null) %>%
          ...
          hist_tail <- hist_full +
            coord_cartesian(xlim = c(x_min, x_max), ylim = c(0, sn_density(as.numeric(u))*1.5)) +
            theme(plot.title = element_blank()) +
            labs(title = sprintf("Stat = %0.4f, p = %0.1e", sqrt(test_ks$statistic), test_ks$p.value)) +
            theme(plot.title = element_text(hjust = 0.5))
        ...
        if(tail == "left"){
          n_trun <- length(z_null_pos[z_null_pos > u_pos])
          expected_quantiles_pos <- -qsn(p = ((2*(1:n_trun)-1)/(2*n_trun))*(1-q),
                                         dp = out@param$dp)
        }else{
          n_trun <- length(z_null_pos[z_null_pos > u_pos])
          expected_quantiles_pos <- qsn(p = 1 - ((2*(1:n_trun)-1)/(2*n_trun))*(1-q),
                                        dp = out@param$dp)
        }
        ...
        if(tail == "left"){
          ...
          observed_tail_prob <- (10:((1-q)*n))/(n)
          observed_extreme_quantile <- quantile(z_null, observed_tail_prob)
          sknorm_tail_prob <- as.vector(psn(x = observed_extreme_quantile,
                                            dp = out@param$dp))
        }else{
          observed_tail_prob <- (10:((1-q)*n))/(n)
          observed_extreme_quantile <- quantile(z_null, 1 - observed_tail_prob)
          sknorm_tail_prob <- (1 - as.vector(psn(x = observed_extreme_quantile,
                                                 dp = out@param$dp)))
        }
        # plot the tail probability
        # observed_tail_prob <- observed_tail_prob*(1-q)
        p_plot <- tibble(observed_tail_prob = as.vector(observed_tail_prob),
                         sknorm_tail_prob = sknorm_tail_prob,
                         observed_prob_ub = observed_tail_prob + 1.96*sqrt(observed_tail_prob*(1-observed_tail_prob)/n),
                         observed_prob_lb = observed_tail_prob - 1.96*sqrt(observed_tail_prob*(1-observed_tail_prob)/n)) %>%
          ggplot(aes(x = sknorm_tail_prob, y = observed_tail_prob)) +
          geom_line() +
          geom_ribbon(aes(ymin = observed_prob_lb, ymax = observed_prob_ub), alpha=0.0,
                      linetype = "dashed", colour="blue")+
          geom_abline(linetype = "dashed", color = "red") +
        ...
        # compute the ratio p-value
        ratio_pvalue <- max(sknorm_tail_prob / observed_tail_prob,
                            observed_tail_prob / sknorm_tail_prob, na.rm = TRUE)
        print(ratio_pvalue)
        # save the gpd parameters and gof statistics and p-value
        gpd_param[[which(names(gpd_param) == tail)]][1:3, l] <- out@param$dp
        gpd_param[[which(names(gpd_param) == tail)]][4, l] <- test_ks$statistic
        gpd_param[[which(names(gpd_param) == tail)]][5, l] <- test_ks$p.value
        gpd_param[[which(names(gpd_param) == tail)]][6, l] <- ratio_pvalue
      }
