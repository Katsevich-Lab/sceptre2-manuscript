...
param$eff_ss <- as.character(rep(eff_ss, 2))

## Left tail (vary with effective sample size)
left_boxplot <- ggplot(param[1:330,], aes(x = eff_ss, y = shape)) + 
  stat_boxplot(geom = "errorbar", # Error bars
               width = 0.25) +    # Bars width
  geom_boxplot()
save_plot(sprintf("%s/left_tail_shape-effss.pdf", file.path(working_dir, subDir)), left_boxplot)
## Right tail (does not vary much with effective sample size)
right_boxplot <- ggplot(param[331:660,], aes(x = eff_ss, y = shape)) + 
  stat_boxplot(geom = "errorbar", # Error bars
               width = 0.25) +    # Bars width
  geom_boxplot()
save_plot(sprintf("%s/right_tail_shape-effss.pdf", file.path(working_dir, subDir)), right_boxplot)
