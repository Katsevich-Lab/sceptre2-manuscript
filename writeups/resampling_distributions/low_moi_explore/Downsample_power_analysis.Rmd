...
undershoot_ame <- data.frame(mae = apply(undershoot_acc, 2, mean),
                             no_sam = rep(no_sam, 10), 
                             ratio_quantile = as.character(round(rep(quantile_list, each = 10), 2)))
undershoot_ame |>
  ggplot(aes_string(x = "no_sam", y = "mae", colour = "ratio_quantile")) +
  scale_x_log10() +
  scale_y_log10() +
  geom_point() +
  geom_line() +
...
overshoot_ame <- data.frame(mae = apply(overshoot_acc, 2, mean),
                            no_sam = rep(no_sam, 10), 
                            ratio_quantile = as.character(round(rep(quantile_list, each = 10), 2)))
overshoot_ame |>
  ggplot(aes_string(x = "no_sam", y = "mae", colour = "ratio_quantile")) +
