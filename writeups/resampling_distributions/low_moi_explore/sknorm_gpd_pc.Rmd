---
title: "GPD vs Skew Normal: pc case"
output:
...
subDir_gpd <- sprintf("%s/tail_prob_%d_pc_resamples_%.2f_percentile", subfld,  samples, q)
subDir_sknorm <- sprintf("%s/sknorm_tail_prob_%d_pc_resamples_%.2f_percentile", subfld, samples, q)
...
param <- tibble(method = c(rep("GPD", 176), rep("Sknorm", 176)),
                tail = rep(c(rep("left", 88), rep("right", 88)), 2),
...
param_comparison <- cbind(param[1:176, 2:6], param[177:352, 3:6]) 
...
Based on the KS test, we find that GPD is a much better fit than skew-normal. Based on the approximation of tail probabilities, a more mixed picture emerges. The GPD tends to *underestimate* the tail probability (by median factors of 2.4 and 1.9 in the left and right tails, respectively). On the other hand, skew-normal tends to *overestimate* the tail probability (by median factors of 1.7 and 1.3 in the left and right tails, respectively).
...
tribble(
  ...
  "Has unbounded support", "Skew-normal",
